...
export const ArticleCard = ({ article, onGradeChange }: ArticleCardProps) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row max-h-40 md:max-h-32">
        {/* Image section - only rendered if imageUrl exists */}
        {article.imageUrl && (
          <div className="h-32 w-full md:h-32 md:w-1/4">
            <img
        ...
        <div className={`flex flex-col p-3 ${article.imageUrl ? 'md:w-3/4' : 'w-full'}`}>
          ...
          <h2 className="mt-1 text-base font-bold leading-tight truncate">
            <Link 
          ...
          {/* Article summary - limited to 1 line */}
          <p className="mt-1 flex-1 text-xs text-muted-foreground line-clamp-1">
            {article.summary}
          </p>
